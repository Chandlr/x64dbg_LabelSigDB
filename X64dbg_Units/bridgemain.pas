...
Const  
	//Bridge defines 有两个dll一个有下划线一个没有
	MAX_SETTING_SIZE = 65536;
	DBG_VERSION = 18;
  {$IFDEF WIN64}
		x32_BRIDGE  = 'x64_bridge.dll';
		x32BRIDGE  = 'x64bridge.dll';
  {$ELSE}
...
{BRIDGE_IMPEXP void*}   function BridgeAlloc(size: LongInt): Pointer; cdecl; external x32_BRIDGE;  //申请的内存
{BRIDGE_IMPEXP void}    procedure BridgeFree(ptr: Pointer); cdecl; external x32_BRIDGE;  //释放申请的内存
{BRIDGE_IMPEXP bool}    function BridgeSettingGet(const section: PAChar; const key:PAChar;value: PAChar): Boolean; cdecl; external x32_BRIDGE;  //读x64_dbg.ini 字符串
{BRIDGE_IMPEXP bool}    function BridgeSettingGetUint(const section: PAChar;const key: PAChar;value: duint): Boolean; cdecl; external x32_BRIDGE;//读x64_dbg.ini 数字
{BRIDGE_IMPEXP bool}    function BridgeSettingSet(const section: PAChar; const key: PAChar; const value: PAChar): Boolean; cdecl; external x32_BRIDGE; //写x64_dbg.ini 字符串
{BRIDGE_IMPEXP bool}    function BridgeSettingSetUint(const section: PAChar; const key: PAChar;value: duint): Boolean; cdecl; external x32_BRIDGE;  //写x64_dbg.ini 数字
...
//fixed by _pusher_
PLBLINFO = ^LBLINFO;
LBLINFO = packed record
    ilabel:array[0..MAX_LABEL_SIZE-1] of AChar;
end;

//Debugger functions
{BRIDGE_IMPEXP char*}   function DbgInit(): PAChar; cdecl; external x32_BRIDGE;
{BRIDGE_IMPEXP bool}    function DbgMemRead(va:duint;dest: PByte;size: duint): Boolean; cdecl; external x32_BRIDGE;  //读内存
{BRIDGE_IMPEXP bool}    function DbgMemWrite(va: duint; const src: PByte;size: duint): Boolean; cdecl; external x32_BRIDGE; //写内存
...
{BRIDGE_IMPEXP bool}    function DbgIsDebugging(): Boolean; cdecl; external x32_BRIDGE;  //是否处于调试状态
{BRIDGE_IMPEXP bool}    function DbgIsJumpGoingToExecute(addr: duint): Boolean; cdecl; external x32_BRIDGE;  //跳转会执行

//fixed _pusher_
{BRIDGE_IMPEXP bool}    function DbgGetLabelAt(addr: duint;segment:SEGMENTREG; ilabel: PLBLINFO): Boolean; cdecl; external x32_BRIDGE; //读标签

{BRIDGE_IMPEXP bool}    function DbgSetLabelAt(addr: duint; const text: PAChar): Boolean; cdecl; external x32_BRIDGE;      //加标签
{BRIDGE_IMPEXP bool}    function DbgGetCommentAt(addr: duint;text: PAChar): Boolean; cdecl; external x32_BRIDGE;//读注释
{BRIDGE_IMPEXP bool}    function DbgSetCommentAt(addr:duint; const text: PAChar): Boolean; cdecl; external x32_BRIDGE; //加注释
{BRIDGE_IMPEXP bool}    function DbgGetBookmarkAt(addr: duint): Boolean; cdecl; external x32_BRIDGE;//读书签
{BRIDGE_IMPEXP bool}    function DbgSetBookmarkAt(addr: duint;isbookmark: Boolean): Boolean; cdecl; external x32_BRIDGE;//加书签
{BRIDGE_IMPEXP bool}    function DbgGetModuleAt(addr: duint;text: PAChar): Boolean; cdecl; external x32_BRIDGE; //地址转模块名
{BRIDGE_IMPEXP BPXTYPE} function DbgGetBpxTypeAt(addr: duint): BPXTYPE; cdecl; external x32_BRIDGE;
{BRIDGE_IMPEXP duint}   function DbgValFromString(const szstring: PAChar): duint; cdecl; external x32_BRIDGE;//解析出一个表达式的结果   "ecx"   "[esp + 4]"
...
{BRIDGE_IMPEXP duint}   function DbgGetBranchDestination(addr: duint): duint; cdecl; external x32_BRIDGE;//获取分支目的地
...
{BRIDGE_IMPEXP bool}    function DbgAssembleAt(addr: duint; const instruction: PAChar): Boolean; cdecl; external x32_BRIDGE;//汇编
{BRIDGE_IMPEXP duint}   function DbgModBaseFromName(const name: PAChar): duint; cdecl; external x32_BRIDGE;   //模块名转基址
{BRIDGE_IMPEXP void}    procedure DbgDisasmAt(addr: duint;instr: PDISASM_INSTR); cdecl; external x32_BRIDGE;//反汇编
...
{BRIDGE_IMPEXP void}    procedure DbgDisasmFastAt(addr: duint; basicinfo: PBASIC_INSTRUCTION_INFO); cdecl; external x32_BRIDGE; //快速反汇编 只检测指令类型
...
{BRIDGE_IMPEXP void}    procedure GuiUpdateRegisterView(); cdecl; external x32_BRIDGE;       //更新寄存器窗口
{BRIDGE_IMPEXP void}    procedure GuiUpdateDisassemblyView(); cdecl; external x32_BRIDGE;    //更新反汇编窗口
{BRIDGE_IMPEXP void}    procedure GuiUpdateBreakpointsView(); cdecl; external x32_BRIDGE;    //更新断点窗口
{BRIDGE_IMPEXP void}    procedure GuiUpdateWindowTitle(const filename: PACHar); cdecl; external x32_BRIDGE;
{BRIDGE_IMPEXP HWND}    function GuiGetWindowHandle(): HWND; cdecl; external x32_BRIDGE;    //x64dbg自己的句柄Hwnd
...
{BRIDGE_IMPEXP void}    procedure GuiScriptSetTitle(const title: PAChar); cdecl; external x32_BRIDGE;   //设置底下提示条
...
{BRIDGE_IMPEXP void}    procedure GuiUpdateDumpView(); cdecl; external x32_BRIDGE;   //更新内存查看窗口
{BRIDGE_IMPEXP void}    procedure GuiUpdateThreadView(); cdecl; external x32_BRIDGE; //更新线程列表窗口
...
{BRIDGE_IMPEXP int}     function GuiMenuAddEntry(hMenu,hEntry: Integer; const title: PAChar): Integer; cdecl; external x32_BRIDGE;
...
{BRIDGE_IMPEXP bool}    function GuiSelectionGet(hWindow: Integer;selection: PSELECTIONDATA): Boolean; cdecl; external x32_BRIDGE;  //获取选中的块
{BRIDGE_IMPEXP bool}    function GuiSelectionSet(hWindow: Integer;const selection: SELECTIONDATA): Boolean; cdecl; external x32_BRIDGE;//设置选中的块
...
type
  ...
  PXREF_RECORD = ^XREF_RECORD;
  XREF_RECORD = packed record
    addr:duint;
    _type: XREFTYPE;
  end;

  PXREF_INFO = ^XREF_INFO;
  XREF_INFO = packed record
    refcount:duint;
    references:PXREF_RECORD;
  end;

{BRIDGE_IMPEXP bool}    function DbgXrefAdd(addr,from:duint):Boolean; cdecl; external x32BRIDGE;
{BRIDGE_IMPEXP bool}    function DbgXrefDelAll(addr:duint):Boolean; cdecl; external x32BRIDGE;
{BRIDGE_IMPEXP bool}    function DbgXrefGet(addr:duint; info:PXREF_INFO):Boolean; cdecl; external x32BRIDGE; //得函数头地址处的所有调用地址
{BRIDGE_IMPEXP size_t}  function DbgGetXrefCountAt(addr:duint):SIZE_T; cdecl; external x32BRIDGE;  //得函数头地址处的调用个数
{BRIDGE_IMPEXP XREFTYPE}function DbgGetXrefTypeAt(addr:duint):XREFTYPE; cdecl; external x32BRIDGE;

{BRIDGE_IMPEXP HANDLE} function DbgGetProcessHandle():THandle; cdecl; external x32BRIDGE; //当前调试程序的THandle
{BRIDGE_IMPEXP HANDLE} function DbgGetThreadHandle():THandle; cdecl; external x32BRIDGE;
{BRIDGE_IMPEXP DWORD} function DbgGetProcessId():DWORD; cdecl; external x32BRIDGE;  //当前调试程序的PID
{BRIDGE_IMPEXP DWORD} function DbgGetThreadId():DWORD; cdecl; external x32BRIDGE;

{BRIDGE_IMPEXP void}    procedure GuiReferenceSetCurrentTaskProgress(progress: Integer;taskTitle:PAnsiChar); cdecl; external x32BRIDGE;
{BRIDGE_IMPEXP void}    procedure GuiSetDebuggeeNotes(text:PAnsiChar); cdecl; external x32BRIDGE;
